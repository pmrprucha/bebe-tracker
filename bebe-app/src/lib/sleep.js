...
// Fonte: Weissbluth, Taking Cara Babies, schedules pediátricos
export function getPlanoByWeeks(semanas) {
  ...
  if (semanas < 12) return { sestas: 4, w1: 50,  d1: 60,  w2: 70,  d2: 60,  w3: 70,  d3: 45,  w4: 60,  d4: 30, wN: 75,  label: '2–3 meses',          desc: '4 sestas, acordar-janela a crescer' }
  ...
  return                    { sestas: 1, w1: 270, d1: 90,  wN: 270, label: '+18 meses',          desc: '1 sesta curta' }
}

export function getSleepQuality(realMins, expectedMins) {
  if (!realMins || !expectedMins) return null
  const diff = realMins - expectedMins
  const pct = diff / expectedMins
...
export function calcSestas(acordouMins, plano, realTimes) {
  ...
  const s1Dur = realTimes.s1_fim && realTimes.s1_ini
    ? timeDiff(realTimes.s1_ini, realTimes.s1_fim) : null
  const s1Fim = realTimes.s1_fim
    ? toMins(realTimes.s1_fim)
    : realTimes.s1_ini ? toMins(realTimes.s1_ini) + (p.d1 || 60) : s1Alvo + (p.d1 || 60)

  result.sestas.push({
    n: 1, alvo: s1Alvo, durAlvo: p.d1,
    ini: realTimes.s1_ini, fim: realTimes.s1_fim,
    durReal: s1Dur, quality: getSleepQuality(s1Dur, p.d1)
  })

  if (numSestas >= 2) {
    const s2Alvo = s1Fim + (p.w2 || 140)
    const s2Dur = realTimes.s2_fim && realTimes.s2_ini
      ? timeDiff(realTimes.s2_ini, realTimes.s2_fim) : null
    const s2Fim = realTimes.s2_fim
      ? toMins(realTimes.s2_fim)
      : realTimes.s2_ini ? toMins(realTimes.s2_ini) + (p.d2 || 90) : s2Alvo + (p.d2 || 90)

    result.sestas.push({
      n: 2, alvo: s2Alvo, durAlvo: p.d2,
      ini: realTimes.s2_ini, fim: realTimes.s2_fim,
      durReal: s2Dur, quality: getSleepQuality(s2Dur, p.d2)
    })

    if (numSestas >= 3) {
      const s3Alvo = s2Fim + (p.w3 || 140)
      const s3Dur = realTimes.s3_fim && realTimes.s3_ini
        ? timeDiff(realTimes.s3_ini, realTimes.s3_fim) : null
      const s3Fim = realTimes.s3_fim
        ? toMins(realTimes.s3_fim)
        : realTimes.s3_ini ? toMins(realTimes.s3_ini) + (p.d3 || 30) : s3Alvo + (p.d3 || 30)

      result.sestas.push({
        n: 3, alvo: s3Alvo, durAlvo: p.d3,
        ini: realTimes.s3_ini, fim: realTimes.s3_fim,
        durReal: s3Dur, quality: getSleepQuality(s3Dur, p.d3)
      })
  ...
  if (result.deitar && result.deitar > 20 * 60 + 30)
    result.alertas.push('Noite pode ficar para tarde (+20:30)')
...
export function timeDiff(ini, fim) {
  if (!ini || !fim) return null
  const diff = toMins(fim) - toMins(ini)
...
export function getWeeks(birthdate) {
  return Math.floor((new Date() - new Date(birthdate)) / (7 * 24 * 3600 * 1000))
}

export function getAgeLabel(birthdate) {
  const dias = Math.floor((new Date() - new Date(birthdate)) / (24 * 3600 * 1000))
  const meses = Math.floor(dias / 30.44)
  const semanas = Math.floor(dias / 7)
  if (meses < 2) return `${semanas} sem`
